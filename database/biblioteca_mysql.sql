...
CREATE TABLE usuarios (
    ...
    tipo_usuario ENUM('ADMINISTRADOR', 'LECTOR') NOT NULL,
...
CREATE TABLE prestamos (
    ...
    creado_por INT, -- ID del bibliotecario que realizó el préstamo
...
-- Trigger para calcular días de retraso al devolver
DELIMITER //
CREATE TRIGGER calcular_retraso_devolucion
AFTER UPDATE ON prestamos
FOR EACH ROW
BEGIN
    IF NEW.fecha_devolucion IS NOT NULL AND NEW.estado = 'DEVUELTO' AND OLD.estado = 'PRESTADO' THEN
        INSERT INTO devoluciones (id_prestamo, fecha_devolucion, dias_retraso, multa_generada, estado_multa)
        SELECT 
            NEW.id_prestamo,
            NEW.fecha_devolucion,
            GREATEST(0, DATEDIFF(NEW.fecha_devolucion, NEW.fecha_vencimiento)),
            GREATEST(0, DATEDIFF(NEW.fecha_devolucion, NEW.fecha_vencimiento)) * 1.0,
            CASE 
                WHEN DATEDIFF(NEW.fecha_devolucion, NEW.fecha_vencimiento) > 0 THEN 'PENDIENTE'
                ELSE 'PAGADA'
            END;
...
-- Trigger para actualizar disponibilidad al realizar préstamo
DELIMITER //
CREATE TRIGGER actualizar_disponibilidad_prestamo
...
-- Trigger para manejar reservas al completarse un préstamo
DELIMITER //
CREATE TRIGGER manejar_reservas_completadas
AFTER UPDATE ON prestamos
FOR EACH ROW
BEGIN
    IF NEW.estado = 'DEVUELTO' AND OLD.estado = 'PRESTADO' THEN
        ...
        WHERE id_reserva = (
            ...
            FROM (
                ...
                WHERE id_libro = NEW.id_libro 
                AND estado = 'ACTIVA'
                ORDER BY posicion_lista ASC, fecha_reserva ASC
                LIMIT 1
            ) AS temp
        );
    END IF;
...
SELECT 
    ...
    p.estado,
    CASE 
        WHEN p.fecha_vencimiento < CURDATE() AND p.estado = 'PRESTADO' 
        THEN 'RETRASADO' 
        ELSE 'AL DIA' 
...
CREATE OR REPLACE VIEW vista_disponibilidad_libros AS
SELECT 
    l.id_libro,
    l.titulo,
    l.autor,
